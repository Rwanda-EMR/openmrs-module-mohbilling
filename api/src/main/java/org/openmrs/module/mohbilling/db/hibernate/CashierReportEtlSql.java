...
final class CashierReportEtlSql {
	...
	static String latestPaymentIdSql() {
		return "SELECT MAX(bill_payment_id) FROM " + TABLE + " WHERE service_id = 0";
	}
	...
	private static String headerInsertSql(boolean dateBounded, boolean paymentIdBounded) {
		...
		sql.append(insertPrefix())
				...
				.append("payment.void_reason, beneficiary.patient_id, primary_identifier.identifier, ")
	...
	private static String serviceInsertSql(boolean dateBounded, boolean paymentIdBounded) {
		...
		sql.append("GROUP BY payment.bill_payment_id, payment.patient_bill_id, payment.collector, ")
				...
				.append("primary_identifier.identifier, ")
	...
	static String reportSql(boolean filterByCollector, boolean filterByPaymentType, boolean filterByServices,
		...
		sql.append("SELECT report_etl.bill_payment_id, report_etl.patient_bill_id, report_etl.date_received, ")
				...
				.append("report_etl.patient_identifier, ")
	...
	private static String insertPrefix() {
		return "INSERT INTO " + TABLE + " (bill_payment_id, patient_bill_id, collector_id, payment_type, "
				+ "date_received, amount_paid, payment_void_reason, patient_id, patient_identifier, beneficiary_name, phone_number, "
	...
	private static String paymentSourceJoins() {
		return "FROM moh_bill_payment payment "
				...
				+ selectedPatientIdentifierJoin()
				+ selectedPersonNameJoin("patient_name", "beneficiary.patient_id");
	}

	private static String selectedPatientIdentifierJoin() {
		return "LEFT JOIN patient_identifier primary_identifier ON primary_identifier.patient_identifier_id = ("
				+ "SELECT identifier.patient_identifier_id FROM patient_identifier identifier "
				+ "WHERE identifier.patient_id = beneficiary.patient_id "
				+ "AND identifier.identifier_type = :primaryIdentifierTypeId "
				+ "AND COALESCE(identifier.voided, 0) = 0 "
				+ "ORDER BY identifier.preferred DESC, identifier.patient_identifier_id DESC LIMIT 1) ";
	}
	...
	private static String updateAssignments() {
		String[] columns = {
				...
				"payment_void_reason", "patient_id", "patient_identifier", "beneficiary_name", "phone_number", "transaction_status",
