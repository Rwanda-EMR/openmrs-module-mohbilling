...
//		List<HopService> reportColumns = GlobalPropertyConfig.getHospitalServiceByCategory(columns);
    ...
    public static BigDecimal getTotalByBillReportItems(List<PatientServiceBillReport> billItems) {
    ...
    public static BigDecimal getTotalByBillReportCategorizedItems(List<PatientServiceBillReport> billItems, String category) {

        BigDecimal totalByCategory = new BigDecimal(0);
        for (PatientServiceBillReport psb : billItems) {
            if (psb.getHopServiceName().equals(category)) {
                BigDecimal reqQty = BigDecimal.valueOf(psb.getServiceBillQuantity());
                BigDecimal unitPrice = BigDecimal.valueOf(psb.getServiceBillUnitPrice());
                totalByCategory = totalByCategory.add(reqQty.multiply(unitPrice));
            }
