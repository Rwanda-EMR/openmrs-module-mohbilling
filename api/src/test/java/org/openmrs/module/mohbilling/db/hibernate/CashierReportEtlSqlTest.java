...
public class CashierReportEtlSqlTest {
	...
	@Test
	public void latestPaymentIdSql_shouldReadOnlyPaymentHeaders() {
		String sql = CashierReportEtlSql.latestPaymentIdSql();

		Assert.assertTrue(sql.contains("MAX(bill_payment_id)"));
		Assert.assertTrue(sql.contains("service_id = 0"));
	}
	...
	public void headerInsertSql_shouldMaterializeEveryNonVoidedPayment() {
		...
		Assert.assertTrue(sql.contains("identifier.identifier_type = :primaryIdentifierTypeId"));
		Assert.assertTrue(sql.contains("primary_identifier.identifier"));
	...
	public void reportSql_shouldPreserveHeadersAndFilterOrdinaryServices() {
		...
		Assert.assertTrue(sql.contains("report_etl.patient_identifier"));
